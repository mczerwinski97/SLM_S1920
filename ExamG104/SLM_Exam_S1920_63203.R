...
# (1p) Divide balanced data into training (80%) and validation (20%) with seed equal to 22
# (1p) Build logistic regression model with 'glm' function. We want to classify 'target' using all other features in a dataset.
# (2p) Conduct feature selection using 'step' function. Use AIC (Akaike Information Criterion) as model selection criterion. How many variables were removed from model compared to initial logistic regression?
# (4p) Using model after feature selection, conduct cost-based threshold optimization on test data with False Positive cost equal to 1 and False Negative Equal to 7. Plot cost-threshold relation and provide optimal cutoff threshold value.
...
# (3p) Prepare the GAM model with the 'mgcv' package, with wine quality as target variable and all other variables as predictors. Apply smoothing the spline only to 'volatile.acidity' variable
# (2p) Prepare a linear regression model with an 'lm' package, with wine quality as target variable and all other variables as predictors. Provide Multiple R-squared of the model.
